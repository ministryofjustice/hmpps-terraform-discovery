...
def update_sc_namespace(ns_id, data):
  try:
    ...
    if not ns_id:
      x = requests.post(
        f'{SC_API_ENDPOINT}/v1/namespaces',
      ...
      x = requests.put(
        f'{SC_API_ENDPOINT}/v1/namespaces/{ns_id}',
        headers=sc_api_headers,
        json={'data': data},
        timeout=10,
      )
    if x.status_code == 200:
      ...
      log.info(
        f'Received non-200 response from service catalogue for namespace id {ns_id}: {x.status_code} {x.content}'
      )
...
def get_sc_product_id(product_id):
  try:
    r = requests.get(
      f'{SC_API_ENDPOINT}/v1/products?filters[p_id][$eq]={product_id}',
      headers=sc_api_headers,
      timeout=10,
    )
    if r.status_code == 200:
      log.info(
        f'Successfully found product with internal ID {product_id}: {r.status_code}'
      )
      return r.json()['data'][0]['id']
    else:
      log.info(
        f'Received non-200 response from service catalogue searching for internal product ID: {product_id}: {r.status_code} {r.content}'
      )
      return False
  except Exception as e:
...
def process_repo(**component):
  for e in component['attributes']['environments']:
    ...
    r = requests.get(
      f'{SC_API_ENDPOINT}/v1/namespaces?{sc_filter_namespace}',
      headers=sc_api_headers,
      timeout=10,
    )
    if r.status_code == 200:
            ...
            if sc_data[0]['attributes']['rds_instance']:
              ...
              rds_id = list(
                filter(
                  ...
                  sc_data[0]['attributes']['rds_instance'],
                )
              )[0]['id']
            ...
            if sc_data[0]['attributes']['elasticache_cluster']:
              ...
              elasticache_id = list(
                filter(
                  ...
                  sc_data[0]['attributes']['elasticache_cluster'],
                )
              )[0]['id']
              ...
              if sc_data[0]['attributes']['pingdom_check']:
                ...
                pingdom_id = list(
                  filter(
                    ...
                    sc_data[0]['attributes']['pingdom_check'],
                  )
                )[0]['id']
